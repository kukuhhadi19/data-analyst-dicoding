...
with st.sidebar:
    # Title
    st.title("Kukuh Hadi S.")

    # Logo Image
    st.image("./dashboard/dashboard.png")
...
sns.barplot(x="product_count", y="product_category_name_english", data=sum_order_items_df.head(5), palette="Set2", ax=ax[0])
ax[0].set_ylabel(None)
ax[0].set_xlabel("Number of Sales", fontsize=30, color="#FF5733")
ax[0].set_title("Produk paling banyak terjual", loc="center", fontsize=50)
ax[0].tick_params(axis ='y', labelsize=35)
ax[0].tick_params(axis ='x', labelsize=30)

sns.barplot(x="product_count", y="product_category_name_english", data=sum_order_items_df.sort_values(by="product_count", ascending=True).head(5), palette="Set2", ax=ax[1])
...
st.caption('Copyright (C) Kukuh Hadi S. 2023')
